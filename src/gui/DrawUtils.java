...
public class DrawUtils {
    ...
    public static boolean isWithinBounds(MouseEvent e, Point p) {
        ...
        return (x <= boundX + size && x >= boundX - size) && (y <= boundY + size && y >= boundY - size);
    }
    ...
    public static boolean isOverlapping(MouseEvent e, Point p) {
        ...
        return (x <= boundX + 2.5*size && x >= boundX - 2.5*size) && (y <= boundY + 2.5*size && y >= boundY - 2.5*size);
    }
    ...
    public void drawHalo(Node node){
        ...
        size+=5;
        g.fillOval(node.getX() - size, node.getY() - size, 2 * size, 2 * size);
        size-=5;
    }
